...
fn check_if_number_is_part_number(number: &Number, symbols: &Vec<Symbol>) -> bool {
    ...
    return symbols.into_iter().any(|symbol| {
        symbol.x >= xmin && symbol.x <= xmax && symbol.y >= ymin && symbol.y <= ymax
    });
}
...
pub fn day3_1() {
    ...
    let valid_numbers: Vec<Number> = field
        .numbers
        .into_iter()
    ...
    println!(
        "Total amount of Symbols and Numbers: {} / {}",
        n_symbols, n_numbers
    );
...
pub fn day3_2() {
    ...
    let gear_ratios: u32 = field
        .symbols
        .into_iter()
        .map(|s| get_gear_ratio_if_gear_otherwise_0(s, &field.numbers))
        .sum();
